...
Since the 2016 election result already came out, I am starting to enrich the original political finance dataset with vote data.  
...
During the election period, candidates need to travel around the cities to win supports. Using the heat map would provide a clear visualization on where the most supportive cities in terms of donation amount.  
...
corrplot(N, method="number", is.corr=FALSE)
...
map_party <- ggmap(map)+
  geom_point(aes(x=longitude, y=latitude, size=contb_receipt_amt, 
                 color = party), 
             data=subset(NewDF, state == "OH"), alpha=1/50)+ 
  scale_colour_manual(values=c("Republican" = "pink", "Democrat" = "blue", "Other" = "grey"))+
