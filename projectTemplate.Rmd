...
========================================================
#### General R library and data loading
...
```{r  Function: Party categorize function, echo=FALSE}
rnParty <- function(x) {
    if(is.na(x)){
      NA
             ...
             x == "Pataki, George E."){
      "R"
    }else if(x == "Clinton, Hillary Rodham" | x == "Kaine, Tim"| x == "Lessig, Lawrence"| x == "O'Malley, Martin Joseph"){
      "D"
    }else{
      "Other"
    }
...
```{r Univariate_Plots: summary of the dataset and know the length of some key attributes, echo=FALSE, message=FALSE, warning=FALSE}
summary(oh_data)
nrow(oh_data)
# Length of key attributes
length(unique(oh_data$cand_nm)) # unique candidate
length(unique(oh_data$contbr_occupation))
length(unique(oh_data$contbr_city))

```

```{r Univariate_Plots: The distribution of contribution amount, echo=FALSE, message=FALSE, warning=FALSE}
...
a + geom_histogram(binwidth = 100, boundary = 0) + # try to make a bin starting from 0
    geom_vline(aes(xintercept=mean(contb_receipt_amt, na.rm=T)), color = "red", linetype = "dashed", size = 1) +
    coord_cartesian(xlim = c(-100, 500)) # zooming to the area of amount from -100 to $500

```

```{r Univariate_Plots: The number of contributions by party, echo=FALSE, message=FALSE, warning=FALSE}
# The number of contributions by candidate
...
print(by_party)
bar <- ggplot(by_party, aes(x= reorder(party, -percent), y = percent, fill = party)) +
...
```{r Univariate_Plots : Top 10 cities with the most number of contributions, echo=FALSE, message=FALSE, warning=FALSE}
...
g1 <- ggplot(d_s, aes(x = contbr_city)) +
  geom_bar()+
  theme_classic()
...
top_city
g2 <- ggplot(top_city, aes(x = reorder(contbr_city, -total_amount), y = total_amount)) +
...
3) weekday: analyzing if there is a huge difference between weekday and wweekend.
...
vote_result <- fread("https://raw.githubusercontent.com/tonmcg/County_Level_Election_Results_12-16/master/2016_US_County_Level_Presidential_Results.csv")
oh_vote <- subset(vote_result, state_abbr == "OH")
simple_oh_vote <- oh_vote[, c(10,2,3,4)]

# Clean Ohio vote data
split_function <- str_split_fixed(simple_oh_vote$county_name, " ", 2)
...
New_by_city <- merge(by_city.wide, simple_oh_vote, by.x = 'contbr_city', by.y = 'county_name')


colnames(New_by_city) <- c("contbr_city", "amount_D", "amount_Other", "amount_R", "votes_D","votes_R","total_votes")
New_by_city$total_amount <- New_by_city$amount_D + New_by_city$amount_R + New_by_city$amount_Other
head(New_by_city)
...
oh_data$Month_Yr <- format (as.Date(oh_data$contb_receipt_dt, "%d-%b-%y"), "%Y-%m")
oh_data$Month_Yr <- as.factor(oh_data$Month_Yr)
oh_data$Day_Month <- format (as.Date(oh_data$contb_receipt_dt, "%d-%b-%y"), "%d")
...
ggplot(top10_occupation, aes(x = reorder(contbr_occupation, -count), y = count))+
  geom_bar(stat = "identity")+
  geom_line(data = top10_occupation, aes(y = average_amount), colour = "blue", group =1 )+
  scale_y_continuous(sec.axis = sec_axis(~./400, name = "Average Amount"))


# two plots
p1 <- ggplot(top10_occupation, aes(x = reorder(contbr_occupation, -count), y = count)) + geom_bar(stat = "identity")  + theme_bw()
p2 <- ggplot(top10_occupation, aes(x = reorder(contbr_occupation, -count), y = count)) +
geom_line(data = top10_occupation, aes(y = average_amount), colour = "blue", group =1 ) + theme_bw() %+replace% 
...
cNamelist <- c("cand_nm", "contbr_city", "contbr_zip", "contb_receipt_amt", "party")
...
r1 <- ggplot(data = New_by_city, aes(x= total_amount, y= total_votes), group = contbr_city)+
  geom_point()+
  geom_smooth(method = "lm", se= FALSE)+
  ggtitle("Plot1-1. Relationship between Total Contributed Amount and Total Votes")


r2 <- ggplot(data = New_by_city, aes(x= amount_R, y= votes_R), group = contbr_city)+
  geom_point()+
  geom_smooth(method = "lm", se= FALSE)+
  ggtitle("Plot1-2. Relationship between Total Contributed Amount and Total Votes of Republican Party")


r3 <- ggplot(data = New_by_city, aes(x= amount_D, y= votes_D), group = contbr_city)+
  geom_point()+
  geom_smooth(method = "lm", se= FALSE)+
...
map_total <- ggmap(map)+
  geom_point(aes(x=longitude, y=latitude, size=contb_receipt_amt), 
             data=subset(NewDF, state == "OH"), alpha=1/150, color = "lightgreen")+
...
map_party <- ggmap(map)+
  geom_point(aes(x=longitude, y=latitude, size=contb_receipt_amt, color = party), 
             data=subset(NewDF, state == "OH"), alpha=1/50)+ 
  scale_colour_manual(values=c("R" = "pink", "D" = "Blue", "Other" = "gray"))+
